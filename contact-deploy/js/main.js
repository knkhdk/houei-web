...
function initSimpleSlider(sliderTrack, type) {
    ...
    // ドットインジケーターのクリックイベント（worksスライダーの場合）
    if (type === 'works') {
        const dots = document.querySelectorAll('.slider-dots .dot');
        dots.forEach((dot, index) => {
            dot.addEventListener('click', (e) => {
                ...
                currentSlide = index;
                updateSlider();
                console.log(`${type}スライダー: ドット${index + 1}がクリックされました`);
            });
        });
        console.log(`${type}スライダー: ドットインジケーターイベント設定完了 (${dots.length}個)`);
    }
    ...
    // 自動スライド機能（事業内容スライダーのみ）
    if (type === 'services') {
        let autoSlideInterval;
        
        // 自動スライドを開始する関数
        function startAutoSlide() {
            if (autoSlideInterval) return; // 既に実行中の場合はスキップ
        ...
        // 手動操作時に自動スライドを一時停止
        const sliderControls = sliderTrack.closest('.services-slider').querySelector('.slider-controls');
        if (sliderControls) {
            const prevBtn = sliderControls.querySelector('.prev-btn');
            const nextBtn = sliderControls.querySelector('.next-btn');
