document.addEventListener('DOMContentLoaded', function() {
    ...
    function submitForm() {
        console.log('submitForm関数が呼び出されました');
        ...
        console.log('送信ボタンが見つかりました:', submitBtn);
        ...
        // 実際の送信処理
        console.log('API送信開始: /api/contact');
        fetch('/api/contact', {
            method: 'POST',
            headers: {
        ...
        .then(result => {
            ...
            if (result.success) {
                ...
                // エラーメッセージを表示
                showErrorMessage(result.error || '送信に失敗しました。');
            }
        })
        .catch(error => {
            ...
            // エラーメッセージを表示
            showErrorMessage('送信に失敗しました。ネットワーク接続を確認してください。');
        });
    }
    ...
    function showErrorMessage(message) {
        ...
        errorMessage.innerHTML = `
            ...
            <h3>送信に失敗しました</h3>
            <p>${message}</p>
            <button class="btn btn-primary" onclick="this.parentElement.remove()">閉じる</button>
        `;
