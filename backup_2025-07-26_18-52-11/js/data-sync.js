...
class DataSync {
    ...
    // インポートされたデータを取得（Node.js不要）
    getImportedData() {
        try {
            const importedData = localStorage.getItem('importedNewsData');
            return importedData ? JSON.parse(importedData) : [];
        } catch (error) {
            console.error('インポートデータ読み込みエラー:', error);
            return [];
        }
    }

    // データをマージ（重複を除去）
    mergeData(localData, fileData) {
        const merged = [...localData];
        
        fileData.forEach(fileItem => {
            const exists = merged.find(localItem => localItem.id === fileItem.id);
            if (!exists) {
    ...
    saveToLocalStorage(data) {
        try {
            ...
            console.log('ローカルストレージに保存完了:', data.length, '件');
            return true;
        } catch (error) {
    ...
    // インポートデータを保存（Node.js不要）
    saveImportedData(data) {
        try {
            localStorage.setItem('importedNewsData', JSON.stringify(data));
            console.log('インポートデータに保存完了:', data.length, '件');
            return true;
        } catch (error) {
            console.error('インポートデータ保存エラー:', error);
            return false;
        }
    }
    ...
    syncData() {
        ...
        // ローカルストレージに保存
        const localSuccess = this.saveToLocalStorage(mergedData);
