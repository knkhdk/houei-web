...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('worksPostForm');
    
    if (form) {
        form.addEventListener('submit', handleWorksSubmit);
    }
    renderWorksAdminList();
});

// 既存施工実績リストの描画
function renderWorksAdminList() {
    const works = JSON.parse(localStorage.getItem('works') || '[]');
    const container = document.getElementById('worksListContainer');
    if (!container) return;
    if (works.length === 0) {
        container.innerHTML = '<p>登録された施工実績はありません。</p>';
        return;
    }
    container.innerHTML = works.map(work => `
        <div class="admin-work-item">
            <div class="admin-work-main">
                <span class="admin-work-title">${work.title}</span>
                <span class="admin-work-category">[${work.category}]</span>
                <span class="admin-work-year">${work.year}年</span>
                <span class="admin-work-location">${work.location}</span>
            </div>
            <div class="admin-work-actions">
                <button class="btn btn-small btn-edit" onclick="editWorkFromAdmin('${work.id}')">編集</button>
                <button class="btn btn-small btn-danger" onclick="deleteWorkFromAdmin('${work.id}')">削除</button>
            </div>
        </div>
    `).join('');
}
...
// 編集ボタン：フォームに反映
window.editWorkFromAdmin = function(id) {
    const works = JSON.parse(localStorage.getItem('works') || '[]');
    ...
    for (let i = 1; i <= 4; i++) {
        document.querySelector(`[name=detail${i}_key]`).value = keys[i-1] || '';
        document.querySelector(`[name=detail${i}_value]`).value = keys[i-1] ? details[keys[i-1]] : '';
    }
    // 編集中IDをセット
    form.setAttribute('data-edit-id', id);
    showMessage('編集モードになりました。修正後「投稿する」で上書き保存されます。', 'info');
}

// 削除ボタン
window.deleteWorkFromAdmin = function(id) {
    if (!confirm('この施工実績を削除しますか？')) return;
    let works = JSON.parse(localStorage.getItem('works') || '[]');
    works = works.filter(w => w.id !== id);
    localStorage.setItem('works', JSON.stringify(works));
    renderWorksAdminList();
    showMessage('施工実績を削除しました', 'success');
}
...
function handleWorksSubmit(e) {
    ...
    let workData = {
        ...
        image: formData.get('image') ? 'works/' + formData.get('image').name : 'works/placeholder.jpg',
        details: {},
        createdAt: editId ? (works.find(w => w.id === editId)?.createdAt || new Date().toISOString()) : new Date().toISOString()
    };
    for (let i = 1; i <= 4; i++) {
        const key = formData.get(`detail${i}_key`);
        const value = formData.get(`detail${i}_value`);
        if (key && value) {
            workData.details[key] = value;
        }
    }
    if (editId) {
        // 編集時は上書き
        works = works.map(w => w.id === editId ? workData : w);
        showMessage('施工実績を更新しました', 'success');
        e.target.removeAttribute('data-edit-id');
    } else {
        // 新規
        works.unshift(workData);
        showMessage('施工実績が正常に投稿されました。', 'success');
    }
    localStorage.setItem('works', JSON.stringify(works));
    renderWorksAdminList();
    e.target.reset();
    ...
    return `
        <div class="works-card preview-card">
            <div class="works-card-image">
                <img src="../images/works/placeholder.jpg" alt="${data.title}">
            </div>
